...
const PropertyImportPage = () => {
  ...
  useEffect(() => {
    const fetchBrokerProfile = async () => {
      ...
      try {
        ...
        const ensured = await ensureBrokerExists(
          user.id,
          user.email,
          user.user_metadata?.name
        );
        if (ensured?.id) setBrokerId(ensured.id);
      } catch (error) {
  ...
  const handleImport = async () => {
    ...
    if (loadingBroker) {
      toast({
        title: "Aguarde",
        description: "Estamos verificando seu perfil antes de importar.",
      });
      return;
    }
    ...
    if (!brokerId) {
      toast({
        ...
        description: "Não foi possível identificar/criar seu perfil de corretor. Tente recarregar a página e fazer login novamente.",
        variant: "destructive"
      });
      return;
    }
