...
const PropertyForm = ({ mode = 'create', initialData = null, onSuccess }) => {
  ...
  const handleImageUpload = async (e, field = 'images') => {
    ...
    try {
      ...
      console.error('[PropertyForm] Upload error:', normalized);

      const devDetails = import.meta.env.DEV
        ? (normalized.message || normalized.details || normalized.hint || normalized.code || JSON.stringify(normalized))
        : null;

      toast({
        title: "Erro no upload",
        description: devDetails ? `Falha ao enviar arquivos. Detalhes: ${devDetails}` : "Falha ao enviar arquivos.",
        variant: "destructive"
      });
    } finally {
