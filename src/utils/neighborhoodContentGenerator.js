...
export const generateNeighborhoodSchema = (neighborhood, properties) => {
  ...
  const title = `Imóveis à Venda e Locação em ${neighborhood}, SP`;
	const description = `Confira as melhores oportunidades de imóveis em ${neighborhood}. Apartamentos, casas e studios para morar ou investir com a curadoria da Radar.`;
	const baseUrl = (import.meta?.env?.VITE_SITE_URL || 'https://rcvend.com.br').replace(/\/$/, '');
	
	// Use first property image or a default fallback
	const image = properties?.[0]?.images?.[0] || `${baseUrl}/logo-gil-corretor-sp.png`;
	const canonical = `${baseUrl}/neighborhood/${neighborhood.toLowerCase().replace(/\s+/g, '-')}`;
  ...
  return {
    ...
    "publisher": {
      ...
      "logo": {
        "@type": "ImageObject",
        "url": `${baseUrl}/logo-gil-corretor-sp.png`
      }
    },
    "mainEntityOfPage": {
      "@type": "WebPage",
...
export const generateNeighborhoodSEO = (neighborhood, properties) => {
  ...
  const baseUrl = (import.meta?.env?.VITE_SITE_URL || 'https://rcvend.com.br').replace(/\/$/, '');
	...
	let title = `Imóveis em ${neighborhood} - São Paulo | Radar`;
	if (hasSales && hasRent) title = `Imóveis à Venda e Locação em ${neighborhood} - SP | Radar`;
	else if (hasSales) title = `Apartamentos à Venda em ${neighborhood} - SP | Radar`;
	else if (hasRent) title = `Apartamentos para Alugar em ${neighborhood} - SP | Radar`;
  ...
  const slug = neighborhood.toLowerCase().replace(/\s+/g, '-');
	const url = `${baseUrl}/neighborhood/${slug}`;
